  ...
  if (!currentUser) {
    ...
    <div class="relative z-10 w-full max-w-sm p-6">
      ...
      <div class="glass rounded-2xl p-6 shadow-2xl border-t border-white/10">
        ...
        <form id="loginForm" action="/login" method="POST" onsubmit="showLoad()"><div class="space-y-4"><div class="relative"><i class="fas fa-user absolute left-3 top-3.5 text-gray-500"></i><input name="username" placeholder="Username" class="w-full pl-10 p-3 rounded-xl input-dark" required></div><div class="relative"><i class="fas fa-lock absolute left-3 top-3.5 text-gray-500"></i><input name="password" type="password" placeholder="Password" class="w-full pl-10 p-3 rounded-xl input-dark" required></div><button class="w-full py-3 rounded-xl gold-bg font-bold shadow-lg text-black">LOGIN NOW</button></div></form>
        <form id="regForm" action="/register" method="POST" class="hidden" onsubmit="showLoad()"><div class="space-y-4"><div class="relative"><i class="fas fa-user-plus absolute left-3 top-3.5 text-gray-500"></i><input name="username" placeholder="Create Username" class="w-full pl-10 p-3 rounded-xl input-dark" required></div><div class="relative"><i class="fas fa-key absolute left-3 top-3.5 text-gray-500"></i><input name="password" type="password" placeholder="Create Password" class="w-full pl-10 p-3 rounded-xl input-dark" required></div><button class="w-full py-3 rounded-xl bg-slate-700 text-white font-bold hover:bg-slate-600">CREATE ACCOUNT</button></div></form>
      </div>
    </div>
    <script>
  ...
  return new Response(`
    ...
    <nav class="glass fixed top-0 w-full z-50 px-4 py-3 flex justify-between items-center shadow-lg">
        <div class="flex items-center gap-2">
           <div class="w-8 h-8 rounded-full gold-bg flex items-center justify-center font-bold text-black text-sm border-2 border-white">${currentUser[0].toUpperCase()}</div>
    ...
    <div class="pt-20 px-4 pb-24 max-w-md mx-auto space-y-6">
        ...
        <div class="space-y-4">
            ...
            <div class="glass p-4 rounded-xl space-y-4">
                ...
                <form action="/admin/payout" method="POST" onsubmit="showLoad()" class="flex gap-2">
                     <select name="session" class="input-dark text-xs"><option value="MORNING">12:01 PM</option><option value="EVENING">04:30 PM</option></select>
                     <input name="win_number" placeholder="Win" class="input-dark w-16 text-center">
                     <button class="bg-red-600 text-white text-xs px-3 rounded font-bold">PAY</button>
                </form>
                
                <form action="/admin/topup" method="POST" onsubmit="showLoad()" class="flex gap-2">
                     <input name="username" placeholder="User" class="input-dark text-xs">
                     <input name="amount" type="number" placeholder="Amt" class="input-dark w-20 text-xs">
                ...
                <form action="/admin/block" method="POST" onsubmit="showLoad()" class="flex gap-2">
                     ...
                     <select name="type" class="input-dark text-xs w-20"><option value="direct">One</option><option value="head">Head</option><option value="tail">Tail</option></select>
                     <input name="val" placeholder="Num" class="input-dark w-16 text-xs text-center">
                     <button class="bg-gray-600 text-white text-xs px-2 rounded font-bold">BLK</button>
                     <button type="submit" name="action" value="clear" class="bg-red-900 text-white text-xs px-2 rounded font-bold">CLR</button>
                </form>

                <form action="/admin/settings" method="POST" onsubmit="showLoad()" class="space-y-2 border-t border-gray-700 pt-2">
                ...
                <form action="/admin/reset_pass" method="POST" onsubmit="showLoad()" class="flex gap-2 border-t border-gray-700 pt-2">
                     <input name="username" placeholder="User" class="input-dark text-xs">
                     <input name="password" placeholder="New Pass" class="input-dark text-xs">
                     <button class="bg-yellow-600 text-white text-xs px-2 rounded font-bold">RESET</button>
                </form>

                 <form action="/admin/add_history" method="POST" onsubmit="showLoad()" class="flex gap-2 border-t border-gray-700 pt-2">
                     <input type="date" name="date" class="input-dark text-xs w-1/3">
                     <input name="morning" placeholder="12:01" class="input-dark text-xs w-1/4">
                     <input name="evening" placeholder="04:30" class="input-dark text-xs w-1/4">
                     <button class="bg-purple-600 text-white text-xs px-2 rounded font-bold">ADD</button>
                </form>
        ...
        <div class="glass rounded-xl p-4">
             <div class="flex justify-between items-center mb-3"><h3 class="font-bold text-gray-300 text-sm">Betting History</h3>${!isAdmin?`<button onclick="clrH()" class="text-xs text-red-400"><i class="fas fa-trash"></i> Clear</button>`:''}</div>
             <div class="space-y-2 max-h-60 overflow-y-auto pr-1">
                 ${bets.length === 0 ? '<div class="text-center text-gray-500 text-xs py-4">No data</div>' : ''}
                 ${bets.map(b => `<div class="flex justify-between items-center p-3 rounded-lg bg-black/20 border-l-2 ${b.status==='WIN'?'border-green-500':b.status==='LOSE'?'border-red-500':'border-yellow-500'}"><div><div class="font-mono font-bold text-lg ${b.status==='WIN'?'text-green-400':b.status==='LOSE'?'text-red-400':'text-white'}">${b.number}</div><div class="text-[10px] text-gray-500">${b.time}</div></div><div class="text-right"><div class="font-mono text-sm font-bold">${b.amount.toLocaleString()}</div><div class="text-[10px] font-bold ${b.status==='WIN'?'text-green-500':b.status==='LOSE'?'text-red-500':'text-yellow-500'}">${b.status}</div></div></div>`).join('')}
             </div>
        </div>
    </div>
    ${navHTML}
